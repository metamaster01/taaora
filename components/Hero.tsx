    ...
    <section className="relative flex min-h-screen flex-col overflow-hidden bg-[#0A0908] text-[#F4F1EA]">
      ...
      <div className="relative z-10 flex flex-1 items-center">
        <div className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-10 px-6 pt-24 lg:grid-cols-[1.5fr_1fr] lg:items-start lg:pt-0">
          ...
          {/* Desktop-only tagline/description/CTA column — untouched from before */}
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.9, delay: 0.2 }}
      ...
      {/* Mobile-only compact corner block — small, tucked bottom-left, replaces the
          desktop column above. Desktop layout above is completely unaffected. */}
      <motion.div
      ...
      >
        <div className="max-w-[340px]">
          ...
          <Link
            href={`/products/${product.slug}`}
            className="mt-4 inline-block rounded-full bg-[#F4F1EA] px-6 py-2.5 text-[10px] font-medium uppercase tracking-[0.15em] text-[#0A0908] transition-all duration-300 hover:scale-105 hover:opacity-90"
          >
            Shop now
          </Link>
        </div>
      ...
      {stripItems.length > 0 && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          transition={{ duration: 0.9, delay: 0.4 }}
          className="relative z-10 mt-auto hidden px-6 pb-10 sm:pb-14 lg:block"
        >
          <div className="mx-auto flex max-w-6xl items-end">
            {stripItems.map((item, i) => {
              const isLast = i === stripItems.length - 1
              return (
                <div key={item.label} className="contents">
                  <div className={`flex items-end gap-3 ${isLast ? 'flex-row-reverse' : ''}`}>
                    {item.image && (
                      <div className="h-24 w-20 overflow-hidden rounded-xl sm:h-28 sm:w-24">
                        <Image
                          src={item.image}
                  ...
                  {i < stripItems.length - 1 && (
                    <span className="mx-4 mb-4 h-px flex-1 bg-white/30" />
                  )}
                </div>
              )
            })}
